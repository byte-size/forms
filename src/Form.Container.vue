  ...
  methods: {
    ...
    displayValidationErrors(errorsObject, inputOrForm) {
      ...
      if (inputOrForm === 'input') {
        ...
        this.domForm.querySelector(`[data-validation-message=${inputName}]`).innerHTML = errorsObject.errors[0]
      } else {
        errorsObject.errors.forEach((errorMessage, index) => {
          const inputName = errorsObject.inner[index].path
          this.domForm.querySelector(`[data-validation-message=${inputName}]`).innerHTML = errorMessage
        })
      }
    },
